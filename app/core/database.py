            ...
            try:
                ...
                for idx, row in df.iterrows():
                    try:
                        ...
                        if validated_record:
                            logger.debug(f"Validated record at row {idx}: {validated_record}")
                            if is_record_complete(validated_record):
                                sheet_records.append(validated_record)
                            else:
                                logger.debug(f"Skipping incomplete record at row {idx}: {validated_record}")
                        else:
                            logger.debug(f"Skipping invalid record at row {idx}: {record}")
                    except Exception as e:
    ...
    try:
        ...
        for field in date_fields:
            if field in clean_record and clean_record[field] is not None:
                try:
                    ...
                    logger.debug(f"Failed to parse date for field {field}: {clean_record[field]}")
        ...
        for field in text_fields:
            if field in clean_record and clean_record[field] is not None:
                ...
                if field == 'father_name' and clean_record[field].lower().startswith('unnamed'):
                    clean_record[field] = None  # Handle 'Unnamed: 9' as NULL
